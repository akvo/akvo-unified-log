(defproject akvo-unified-log "0.1.0-SNAPSHOT"
  :description "FIXME: write description"
  :url "http://github.com/akvo/akvo-unified-log"
  :license {:name "GNU Affero General Public License"
            :url "https://www.gnu.org/licenses/agpl"}
  :plugins [[lein-environ "1.0.0"]]
  :dependencies [[org.clojure/clojure "1.7.0"]
                 [org.clojure/java.jdbc "0.4.2"]
                 [org.clojure/core.async "0.1.346.0-17112a-alpha"]
                 [org.akvo/commons "0.4.0-SNAPSHOT"]
                 [org.postgresql/postgresql "9.3-1102-jdbc41"]
                 [com.github.fge/json-schema-validator "2.2.6"]
                 [com.google.appengine/appengine-tools-sdk "1.9.9"]
                 [com.google.appengine/appengine-remote-api "1.9.9"]
                 [com.google.appengine/appengine-api-1.0-sdk "1.9.9"]
                 [ring/ring-core "1.4.0" :exclusions [joda-time]]
                 [ring/ring-json "0.4.0" :exclusions [cheshire joda-time]]
                 [ring/ring-jetty-adapter "1.4.0" :exclusions [joda-time]]
                 [com.taoensso/timbre "4.1.4" :exclusions [org.clojure/tools.reader]]
                 [clj-http "1.0.1" :exclusions [com.fasterxml.jackson.core/jackson-databind
                                                com.fasterxml.jackson.core/jackson-annotations]]
                 [liberator "0.12.2"]
                 [compojure "1.4.0" :exclusions [joda-time]]
                 [yesql "0.5.1" :exclusions [org.clojure/java.jdbc ]]
                 [cheshire "5.5.0"]
                 [environ "1.0.0"]
                 [clj-time "0.9.0" :exclusions [joda-time]]
                 [clj-statsd "0.3.11"]
                 [com.stuartsierra/component "0.3.0"]]
  ;; TODO figure out :profiles {:dev {:source-paths ["dev"]}}
  :source-paths ["dev" "src"])
